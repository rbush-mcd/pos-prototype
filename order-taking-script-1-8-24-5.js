document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleDrinkSelectionClick(event) {
        ...
        if (!drinkName || !drinkImageSrc) return;
    ...
    // Function to handle meal item click
    function handleMealItemClick(event) {
        ...
        if (mealItem.classList.contains('is--meal-side')) {
            // Navigate to item-customization and display French Fries customization content
            switchToCustomizationStep('item-customization', 'French Fries&&&&&');
        } else if (mealItem.id === 'meal-item-drink') {
            // Navigate to item-customization and display the corresponding drink customization content
            const drinkName = mealItem.querySelector('.menu-item-name').innerText;
            switchToCustomizationStep('item-customization', drinkName);
        } else if (mealItem.id === 'happy-meal-item-toy') {
            // Navigate to hm-toy-selection
            switchTab(4); // Assuming tab index 4 corresponds to #hm-toy-selection
        } else {
    ...
    // Add event listeners to meal items
    function addMealItemListeners() {
        const mealItems = document.querySelectorAll('.meal-item');
        if (mealItems.length === 0) {
            console.log('No meal items found');
        } else {
            mealItems.forEach(item => {
                console.log(`Adding click listener to meal item: ${item.getAttribute('data-name')}`);
                item.removeEventListener('click', handleMealItemClick); // Remove existing listener
                item.addEventListener('click', handleMealItemClick); // Add new listener
            });
        }
    }

    // Initial setup
    addMealItemListeners();
